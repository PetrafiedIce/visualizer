...
function SpectrumField({ text }: { text: string }) {
  const points = useMemo(() => new THREE.IcosahedronGeometry(1.5, 3).attributes.position.array as ArrayLike<number>, [])
  const colorA = useMemo(() => stringToColor(text || 'A').getStyle(), [text])
  const colorB = useMemo(() => stringToColor(text.split('').reverse().join('') || 'B').getStyle(), [text])
...
export default function TextSpectrum() {
  const [text, setText] = useState('Type to create a spectrum')
